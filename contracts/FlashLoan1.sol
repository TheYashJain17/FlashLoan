...
contract FlashLoan1{
    ...
    address private constant PANACAKE_ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    ...
    function checkResult(uint256 amountWeGot , uint256 amountHaveToRepay) pure private returns(bool){

        return amountWeGot > amountHaveToRepay;

    }

    function getBalanceOfTokens(address tokenAddress) view external returns(uint256){
    ...
    function placeTrade(address token1 , address token2 , uint256 amountIn) private returns(uint256){
        ...
        require(liquidityPool != address(0) , "Coudnt Find The Pool");
        ...
        uint256 expectedAmount = IUniswapV2Router01(PANCAKE_FACTORY).getAmountsOut(amountIn, path)[1];

        uint256 actualAmount = IUniswapV2Router01(PANACAKE_ROUTER).swapExactTokensForTokens(amountIn, expectedAmount, path, address(this), deadline)[1];

        require(actualAmount > 0 , "Transaction Failed");
    ...
    function initiateArbitrage(address _borrowingBUSD , uint256 _amountOfBorrowing) external{
        ...
        IERC20(BUSD).safeApprove(address(PANACAKE_ROUTER) , MAX_INT);
        IERC20(CROX).safeApprove(address(PANACAKE_ROUTER) , MAX_INT);
        IERC20(CAKE).safeApprove(address(PANACAKE_ROUTER) , MAX_INT);


        address liquidityPool = IUniswapV2Factory(PANCAKE_FACTORY).getPair(_borrowingBUSD, WBNB);

        require(liquidityPool != address(0) , "Liquidity Pool doesnt Exist");
    ...
    function pancakeCall(uint256 amount0 , uint256 amount1 , address _sender , bytes calldata _data) external {
        ...
        uint256 repaymentAmount = fee + _amountOfBorrowing;
        ...
        IERC20(BUSD).transfer(myAccount , repaymentAmount - trade3Coin);
